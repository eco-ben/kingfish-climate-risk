...
csv <- list.files(path = 'Results/mCRS-DeltamCRS RAW/depth-together/')
csv = csv[grep("SpL", csv)]
CRS.dt <- data.table()
...
for(i in 1:length(csv)){
  ...
  # It should be clear that for analyses labelled 'SpL' PopID only has one value (Seriola lalandi) as this is the relevant population level for that analysis
...
write.csv(PremeanDT, file = 'Results/mCRS-DeltamCRS RAW/DeltaCRS_mCRS_RAW_depth_together_SpL.csv', row.names = F)
write.csv(CRS.dt, file = 'Results/mCRS-DeltamCRS RAW/DeltaCRS_PopValues_depth_together_SpL.csv', row.names = F)
