...
names(PremeanDT.Year)[names(PremeanDT.Year) == "Extract_ID"] <- "Extract.ID"
...
SpL <- data.table(SpL)
names(SpL)[names(SpL) == "Extract_ID"] <- "Extract.ID"
...
surface = dplyr::left_join(surface, Coordinates[, c(9, 1, 2, 3, 4)], by=c("Extract.ID", "PopID"))
surface$Distance_from_equator = ifelse(surface$Alt.Latitude < 0, -surface$Alt.Latitude, surface$Alt.Latitude)
surface$Depth = as.factor(surface$Depth)
surface$PopID = as.factor(surface$PopID)
surface$Level <- "surface_only"
surface_and_depths = rbind(surface, PremeanDT.Year)

ggplot() + 
  geom_boxplot(data = surface_and_depths[surface_and_depths$Scenario == "ssp85" & surface_and_depths$Year == 2100,],
               aes(x = Depth, y = CRS, fill = Level)) + 
  facet_wrap(~PopID)
...
Warmlevel.data$randoff <- sample(1:2436,2436, replace = F)
...
minNY = min(Novel.Year$Novel.Year, na.rm=T)
...
ggplot() +
  ...
  scale_y_continuous(limits = c(2000,2101))+ylab('Timing of regional risk emergence (Year)') +
...
ggplot() +
  ...
  scale_y_continuous(limits = c(2000,2101))+ylab('Timing of regional risk emergence (Year)') +
  facet_wrap(~Depth, nrow = 1) + theme_classic() + xlab('Population') + ggtitle('Regional risk emergence timing across populations - SSP1-2.6') + 
...
ggplot() +
  ...
  geom_jitter(data = Novel.Year[Novel.Year$Scenario == 'ssp45',],
              aes(x = PopLab, y = Novel.Year),color = 'grey', width = 0.3) +
  scale_y_continuous(limits = c(2000,2101))+ylab('Timing of regional risk emergence (Year)') +
  facet_wrap(~Depth, nrow = 1) + theme_classic() + xlab('Population') + ggtitle('Regional risk emergence timing across populations - SSP2-4.5') + 
  labs(fill = 'Population') +  
  theme(axis.text = element_text(size = 13), axis.title = element_text(size = 14, face = 'bold'), 
        strip.text.x = element_text(size = 10)) 
...
ggplot() +
  ...
  geom_jitter(data = Novel.Year[Novel.Year$Scenario == 'ssp85',],
              aes(x = PopLab, y = Novel.Year),color = 'grey', width = 0.3) +
  scale_y_continuous(limits = c(2000,2101))+ ylab('Timing of regional risk emergence (Year)') +
  facet_wrap(~Depth, nrow = 1) + theme_classic() + xlab('Population') + ggtitle('Regional risk emergence timing across populations - SSP5-8.5') + 
  labs(fill = 'Population') + 
  theme(axis.text = element_text(size = 13), axis.title = element_text(size = 14, face = 'bold'), 
        strip.text.x = element_text(size = 10)) 
...
Test2 <- ggplot()+
  ...
  scale_color_viridis_c(limits = c(2001,2100)) + labs(color = 'Timing of regional risk emergence (Year)') + theme(legend.position = 'bottom')
...
Full$PopLab <- ifelse(Full$PopLab == 'South_Africa', 'SA', Full$PopLab)
Full$PopLab <- ifelse(Full$PopLab == 'South_Pacific', 'SIP', Full$PopLab)
...
env$Scenario <- ifelse(env$Date >= '2001-01-01' & env$Date <= '2014-12-31', 'ssp85', env$Scenario)
...
all_pca = left_join(all_pca, Coordinates[,c(9,10)], by='Extract.ID')
...
for (m in 1:length(sp)) {
  ...
  env.poly <- distfree.cr(env.coords[,c(6,7)],alpha = 0.05, draw = F)
  ...
  spat.pcs <- Full[Full$PopLab == sp[m] & Full$Date <= refyear_end,]
  ...
  all_centroids = rbind(all_centroids, env.centroid)
      ...
      year_pca = all_pca %>% group_by(Extract.ID,Depth, PopLab, Scenario, Year) %>%
