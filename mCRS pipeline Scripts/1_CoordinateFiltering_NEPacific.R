...
CombDataNE <- CombData[CombData$PopID == 'NE_Pacific',]
...
coordinates <- CombDataNE[,c(8,1,2)]
...
increase_length <- 2500  # Replace with your desired value
...
#   coord_sf(xlim = c(-150,-50), ylim = c(0,50))

CombDataNE <- dplyr::left_join(CombDataNE,All.points[,c('ID','Alt.Longitude','Alt.Latitude')], by = c('ID'))

#csv for filtered coordinates with likely useful information
write.csv(CombDataNE[,c(9,10,4,5,3,6,7,8)], file = "Data/NEPacific_Coordinates.csv", row.names = F)
