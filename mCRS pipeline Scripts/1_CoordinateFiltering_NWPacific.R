...
CombDataNW <- CombData[CombData$PopID == 'NW_Pacific',]
...
coordinates <- CombDataNW[,c(8,1,2)]
...
increase_length <- 30  # Replace with your desired value
...
CombDataNW <- dplyr::left_join(CombDataNW,All.points[,c('ID','Alt.Longitude','Alt.Latitude')], by = c('ID'))

#csv for filtered coordinates with likely useful information
write.csv(CombDataNW[,c(9,10,4,5,3,6,7,8)], file = "Data/NWPacific_Coordinates.csv", row.names = F)
