...
CombDataSA <- CombData[CombData$PopID == 'South_Africa',]
...
coordinates <- CombDataSA[,c(8,1,2)]
...
increase_length <- 50  # Replace with your desired value
...
#   geom_sf(data = old.points, size = 0.9) +
#   coord_sf(xlim = c(0,50), ylim = c(0,-50))

CombDataSA <- dplyr::left_join(CombDataSA,All.points[,c('ID','Alt.Longitude','Alt.Latitude')], by = c('ID'))

#csv for filtered coordinates with likely useful information
write.csv(CombDataSA[,c(9,10,4,5,3,6,7,8)], file = "Data/SouthAfrica_Coordinates.csv", row.names = F)
