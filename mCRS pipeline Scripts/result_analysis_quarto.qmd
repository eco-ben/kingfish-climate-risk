...
ggsave("kingfish_data_dir/Results/Plots/env_change_10m.png", env_change_10m)
pca_change_10m <- results_and_pca_plot(coordinates, SSP245_env_data, SSP245_warm_level_data, 10, "SSP245", "DeltaCRS", "DeltaCRS", "PCA Change 10m SSP2-4.5")
ggsave("kingfish_data_dir/Results/Plots/pca_change_10m.png", pca_change_10m)

env_change_50m <- plot(env_change_map(SSP245_50m_data, "DeltaCRS", "\u0394mCRS", "Environmental Change 50m SSP2-4.5"))
ggsave("kingfish_data_dir/Results/Plots/env_change_50m.png", env_change_50m)
pca_change_50m <- results_and_pca_plot(coordinates, SSP245_env_data, SSP245_warm_level_data, 50, "SSP245", "DeltaCRS", "DeltaCRS", "PCA Change 50m SSP2-4.5")
ggsave("kingfish_data_dir/Results/Plots/pca_change_50m.png", pca_change_50m)

env_change_100m <- plot(env_change_map(SSP245_100m_data, "DeltaCRS", "\u0394mCRS", "Environmental Change 100m SSP2-4.5"))
ggsave("kingfish_data_dir/Results/Plots/env_change_100m.png", env_change_100m)
pca_change_100m <- results_and_pca_plot(coordinates, SSP245_env_data, SSP245_warm_level_data, 100, "SSP245", "DeltaCRS", "DeltaCRS", "PCA Change 100m SSP2-4.5")
ggsave("kingfish_data_dir/Results/Plots/pca_change_100m.png", pca_change_100m)

env_change_150m <- plot(env_change_map(SSP245_150m_data, "DeltaCRS", "\u0394mCRS", "Environmental Change 150m SSP2-4.5"))
ggsave("kingfish_data_dir/Results/Plots/env_change_150m.png", env_change_150m)
pca_change_150m <- results_and_pca_plot(coordinates, SSP245_env_data, SSP245_warm_level_data, 150, "SSP245", "DeltaCRS", "DeltaCRS", "PCA Change 150m SSP2-4.5")
ggsave("kingfish_data_dir/Results/Plots/pca_change_150m.png", pca_change_150m)
```
...
ggplot() + geom_violin(data = warmlevel_data_all_depths, aes(x = Scenario, y = DeltaCRS, fill=Scenario), draw_quantiles = 0.5) +   geom_jitter(data = warmlevel_data_all_depths, aes(x=Scenario, y = DeltaCRS), alpha=0.1) +
  facet_grid(PopID ~ Depth)
...
ggplot() + geom_bar(data = prop_deltacrs_greater_0, aes(x = Scenario, y = Proportion, fill=Scenario), stat="identity") + 
  facet_wrap(Population~Depth)
```

# `{r stat-analysis depth live-stage} # emission_scen_model= nlme::lme(DeltaCRS ~ Scenario, random = ~ 1|Depth/Extract.ID,  #   data = warmlevel_data_all_depths # ) # summary(emission_scen_model) #`